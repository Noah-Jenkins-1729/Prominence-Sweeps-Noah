...
#        if len(props["peak_heights"]) > 0: # Added by Noah
#            index = np.argmax(props['prominences']) # Added by Noah
#            peaks = [peaks[index]] # Added by Noah
#            for key in props.keys(): # Added by Noah
#                props[key] = [props[key][index]] # Added by Noah
        for peak in peaks:
...
def fit_peaks_multigauss(
    ...
    model.set_param_hint('l_decay', value = centers[1], min = 0) #constraint the slope fit to be less or equal to 0
    model.set_param_hint('l_amplitude', value = counts[0], min = 0)
    params = model.make_params()
    # params.pretty_print()
...
class WaveformProcessor_hacked:
    def __init__(
        self,
    ...
    def get_spe(self) -> Tuple[float, float]:
        ...
        Returns:
            Tuple[float, float]: A tuple containing the decay value and its error.
        """
    ...
    def plot_spe(
        ...
        textstr += f"Prominence: {self.prominence} [V]\n"
        textstr += f"--\n"
        textstr += f"""Decay: {self.spe_res.params['slope'].value:0.4} +- {self.spe_res.params['slope'].stderr:0.2} [1/V]\n"""
        textstr += f"""Amplitude: {self.spe_res.params['intercept'].value:0.4} +- {self.spe_res.params['intercept'].stderr:0.2}\n"""
    ...
    def plot_peak_histograms(
        ...
        plt.plot(centers, self.peak_fit.best_values['l_amplitude']*np.exp(-1*centers/self.peak_fit.best_values['l_decay']), 'b-', label='best fit - line')     
        plt.grid(True)
        props = dict(boxstyle='round', facecolor='tab:' + peakcolor, alpha=0.4)
