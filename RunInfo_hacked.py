...
#                        props[key] = [props[key][index]] # Added by Noah
            ...
            if self.do_peak_find:
                for peak,index in zip(peaks,range(len(peaks))):
                    all_peaks.append(props['prominences'][index]) #edited by Noah
                    all_times.append(time[peak]) #added by Noah
                    if self.led:
                        curr_time = time[peak]
                        if curr_time < led_time_thresh:
                            dark_peaks.append(props['prominences'][index])
                        else:
                            led_peaks.append(props['prominences'][index])
        ...
        if self.led:
            ...
            return all_peaks, dark_peaks, led_peaks, all_times #edited by Noah
        else:
            return all_peaks, all_times #edited by Noah
    ...
    def get_peak_data(self):
        ...
        for curr_file in self.hd5_files:
            ...
            for curr_acquisition_name in self.acquisition_names[curr_file]:
                ...
                if self.led:
                    curr_peaks, curr_dark_peaks, curr_led_peaks,_ = self.get_peaks(curr_file, curr_acquisition_name) #edited by Noah
                else:
                    curr_peaks,_ = self.get_peaks(curr_file, curr_acquisition_name)#edited by Noah
    ...
    def get_peak_data_solicit(self) -> None:
        ...
        for curr_file in self.hd5_files:
            ...
            for curr_acquisition_name in self.acquisition_names[curr_file]:
                ...
                self.all_peak_data = self.all_peak_data + curr_peaks #allows data to be plotted with plot_hists()
                self.peak_data[curr_file][curr_acquisition_name] = curr_peaks
